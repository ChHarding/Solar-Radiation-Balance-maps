# Global Radiation maps (monthly)
# for Diane Thatcher (Geol 106)
...
# This version creates MONTHLY averages
...
# variables to plot, can be only one but must ofc be
#varnames = [ "ttr", "tsr", "str", "ssr"] 
varnames = [ "tsr"] # variables to plot
...
for varname in varnames:
    ...
    vl = np.array(v[varname]).astype(np.int64) # initially, the values are integers (cast to int64)
    print "data values", vl.shape, vl.dtype    # we will later scale  and offset them to get floats
    ...
    #
    # Create monthly means from vl
    #

    # create a list with info about each months 
    mthinfo = [] 
    for m in range(9,13) + range(1, 9): # data starts in sept
	currm = [m,datetime.date(1900, m, 1).strftime('%B'),0] # month num, name, num_slices
	mthinfo.append(currm)
    
    # array for accumulating monthly averages    
    monthly_avg = np.zeros((12, vl.shape[1], vl.shape[2]), dtype=float)
    #print monthly_slices.shape
    
    i = 0 # index in monthly_avg
    for t in vl: # iterate through all time (dim=0) slices
	monthly_avg[i] += t # accumulate monthly values
	mthinfo[i][2] += 1 # [2] is count number of slices accumulated for this month
	#print t.mean(),mthinfo[i], np.mean(monthly_avg[i]/mthinfo[i][2])
	i += 1
	if i > 11: i = 0
    for i,t in enumerate(monthly_avg):
	monthly_avg[i] /= float(mthinfo[i][2]) # divide accumulated by number of slices
	#print mthinfo[i], monthly_avg[i].min(),monthly_avg[i].max()
	#print mthinfo[i]; b,v = np.histogram(t);print b;print v; print
    print
	
    # to get same min/max color for all month
    global_minimum = monthly_avg.min()
    global_maximum = monthly_avg.max()
    print "monthly avg. from", global_minimum, "to", global_maximum    # this is different that min/max of vl
        
    # add one column, otherwise we get a gap on leftmost lon
    from mpl_toolkits.basemap import addcyclic
    monthly_avg,lon = addcyclic(monthly_avg, lon) # works also for array of 12 means
    x, y = np.meshgrid(lon, lat)  # makes 2D arrays from 1D lat and 1D lon    
    
    # Loop over all 12 month      
    for currmth in mthinfo:
                  
        month_name = currmth[1]
        month_as_num = currmth[0]
        mean = monthly_avg[month_as_num-1]  # the mean of the month to plot
	...
	#xmin -= xres/2.0 # 
	ymin += yres/2.0 # I assume the data is cell centered in lat (hence the 73), I so offset by 1/2 a cell
	#print "geotransform:", xmin, xres, ymin, yres   
	geotransform=(xmin,xres,0,ymin,0, -yres)     #
    
	fn = varname + "_" + "%02d" % month_as_num + "_mean.tif" # name of geotiff file
	...
	#
	# Plot mean via matplotlib/Basemap
	#
	...
	""" # as we will use a world wide projection (robinson), we don't need to set a bounding box
	    # However, other projection may require it, so I left if
	...
	for med in medians:        
		...
		# draw data (2-D array) as colored cells
                ...
                #cb_ticks = range(0,100,10) + range(100,325,25)
                cb_ticks = range(-1000,1000,20)
                cbar = m.colorbar(pcm,location='bottom',pad="10%", ticks=cb_ticks)
                cbar.set_label('W / m**2')    
                
                # contour plot
                intv = 40
                if varname == "ttr": intv = 20
                intervals = range(-1000,1000,intv) 
                ...
                title_str = "Annual (1957 - 2002) mean of "+ ncattr["long_name"] + " for " + month_name
		...
		#outfilename  = varname + "_month=" + "%02d" % month_as_num + "_mrd=" + str(med) + "_cmap="+cmap.name  
		outfilename  = varname + "_%03d" % (med+180) + "_" + "%02d" % month_as_num
		
		#outfilename += ".pdf"
		outfilename += ".jpg"
		outfilename = r"C:\temp\gif\" + os.sep + outfilename
